...
export function ManagementCenter({ open, onClose, token, isSystemAdmin, manageableProjects }: Props) {
  ...
  const membersTab = <>
    ...
    <Table size="small" rowKey="username" dataSource={filteredMembers} pagination={false} scroll={{ x: 760 }} columns={[
      ...
      { title: "操作", render: (_, item: Member) => <Space wrap><Button size="small" type="link" onClick={() => openMember("edit", item)}>编辑</Button><Button size="small" type="link" onClick={() => openMember("password", item)}>重置密码</Button><Button size="small" type="link" onClick={() => void submit(`/api/admin/members/${item.username}/status`, "POST", { is_active: !item.is_active })}>{item.is_active ? "停用" : "启用"}</Button></Space> },
    ]} />
  </>;
  ...
  const projectsTab = <>
    ...
    <Table size="small" rowKey="project_id" dataSource={filteredProjects} pagination={false} scroll={{ x: 860 }} columns={[
      ...
      { title: "操作", render: (_, item: Project) => <Space wrap>{item.status === "active" && <><Button size="small" type="link" onClick={() => openProject("edit", item)}>编辑</Button><Button size="small" type="link" onClick={() => openProject("owner", item)}>转移负责人</Button><Button size="small" type="link" onClick={() => openProject("permissions", item)}>权限</Button></>}{isSystemAdmin && <Button size="small" type="link" onClick={() => void submit(`/api/admin/projects/${item.project_id}/${item.status === "active" ? "archive" : "restore"}`, "POST")}>{item.status === "active" ? "归档" : "恢复"}</Button>}</Space> },
    ]} />
  </>;
  ...
  return <section className="management-center">
    ...
    <Drawer title={projectDrawer === "create" ? "新增项目" : projectDrawer === "owner" ? "转移负责人" : projectDrawer === "permissions" ? "项目权限" : "编辑项目"} width={460} open={projectDrawer !== null} onClose={() => setProjectDrawer(null)}>
      {projectDrawer === "owner" ? <Form autoComplete="off" form={ownerForm} layout="vertical" onFinish={async (values) => { if (selectedProject && await submit(`/api/admin/projects/${selectedProject.project_id}/owner`, "POST", values)) setProjectDrawer(null); }}><Form.Item label="新负责人" name="owner_username" rules={[{ required: true, message: "请选择负责人" }]}><Select showSearch optionFilterProp="label" options={members.filter((member) => member.is_active).map((member) => ({ value: member.username, label: `${member.display_name} (${member.username})` }))} /></Form.Item><Button htmlType="submit" type="primary">转移</Button></Form> : projectDrawer === "permissions" ? <><Form autoComplete="off" form={permissionForm} layout="vertical" onFinish={async (values) => { if (selectedProject && await submit(`/api/admin/projects/${selectedProject.project_id}/permissions/${values.username}`, "PUT", { role: values.role })) { permissionForm.resetFields(); void loadPermissions(selectedProject.project_id); } }}><Space.Compact block><Form.Item name="username" rules={[{ required: true, message: "请选择成员" }]} style={{ flex: 1, marginBottom: 12 }}><Select showSearch optionFilterProp="label" placeholder="选择成员" options={members.filter((member) => member.is_active).map((member) => ({ value: member.username, label: `${member.display_name} (${member.username})` }))} /></Form.Item><Form.Item name="role" rules={[{ required: true, message: "请选择角色" }]} style={{ marginBottom: 12 }}><Select placeholder="角色" style={{ width: 130 }} options={ROLE_OPTIONS} /></Form.Item><Button htmlType="submit" type="primary">授权</Button></Space.Compact></Form><Table size="small" rowKey="username" dataSource={permissions} pagination={false} columns={[{ title: "成员", render: (_, item: Permission) => `${item.display_name} (${item.username})` }, { title: "角色", render: (_, item: Permission) => <Select aria-label={`${item.username} 的项目角色`} size="small" value={item.role} style={{ width: 130 }} options={ROLE_OPTIONS} onChange={async (role) => { if (selectedProject && await submit(`/api/admin/projects/${selectedProject.project_id}/permissions/${item.username}`, "PUT", { role })) void loadPermissions(selectedProject.project_id); }} /> }, { title: "操作", render: (_, item: Permission) => <Button danger size="small" onClick={async () => { if (selectedProject && await submit(`/api/admin/projects/${selectedProject.project_id}/permissions/${item.username}`, "DELETE")) void loadPermissions(selectedProject.project_id); }}>撤销</Button> }]} /></> : <Form autoComplete="off" form={projectForm} layout="vertical" onFinish={async (values) => { const saved = projectDrawer === "create" ? await submit("/api/admin/projects", "POST", values) : selectedProject && await submit(`/api/admin/projects/${selectedProject.project_id}`, "PATCH", values); if (saved) setProjectDrawer(null); }}>
        {projectDrawer === "create" && <><Form.Item label="项目标识" name="project_id" rules={[{ required: true, message: "请输入项目标识" }]}><Input /></Form.Item><Form.Item label="负责人" name="owner_username" rules={[{ required: true, message: "请选择负责人" }]}><Select showSearch optionFilterProp="label" options={members.filter((member) => member.is_active).map((member) => ({ value: member.username, label: `${member.display_name} (${member.username})` }))} /></Form.Item></>}
